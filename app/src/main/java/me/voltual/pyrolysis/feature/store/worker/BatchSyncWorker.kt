...
import me.voltual.pyrolysis.ARG_SYNC_REQUEST
...
class BatchSyncWorker(
    ...
    private suspend fun updateSyncProgress(
        ...
        if (state == SyncState.SYNCING && progress != null) {
        // 将 Stage 的枚举名称和索引传出去
        setProgress(workDataOf(
            "progress_stage" to progress.stage.name, // 例如 "DOWNLOAD"
            "progress_index" to progress.stage.ordinal + 1, // 当前第几步
            "total_stages" to RepositoryUpdater.Stage.entries.size, // 总步数
            "repoId" to repoId
        ))
    }
