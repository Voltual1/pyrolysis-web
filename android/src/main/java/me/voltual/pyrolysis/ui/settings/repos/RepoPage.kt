...
import me.voltual.pyrolysis.ui.components.BlockText
import me.voltual.pyrolysis.ui.components.CheckChip
...
import me.voltual.pyrolysis.core.ui.icons.phosphor.GearSix
...
import me.voltual.pyrolysis.ui.dialog.ActionsDialogUI
import me.voltual.pyrolysis.ui.dialog.BaseDialog
import me.voltual.pyrolysis.ui.dialog.DIALOG_NONE
import me.voltual.pyrolysis.ui.dialog.ProductsListDialogUI
import me.voltual.pyrolysis.ui.dialog.StringInputDialogUI
...
import me.voltual.pyrolysis.core.utils.extension.text.pathCropped
import me.voltual.pyrolysis.core.utils.getLocaleDateString
import me.voltual.pyrolysis.core.utils.shareText
...
fun RepoPage(
    ...
    viewModel: RepoPageVM = koinPyrolysisViewModel(),
    onDismiss: () -> Unit,
) {
...
//    val syncInfo by viewModel.syncWorkInfo.collectAsStateWithLifecycle(initialValue = null)
...
AnimatedVisibility(visible = state.isSyncing) { // 建议统一使用 state.isSyncing
    Column(
        modifier = Modifier
            .fillMaxWidth()
        ...
        Row(
            ...
            // 
            val stageDescription = when(state.currentStage) {
                "DOWNLOAD" -> "正在下载索引文件..."
                "PROCESS"  -> "正在解析仓库内容..."
                "MERGE"    -> "正在对比差异..."
                "COMMIT"   -> "正在保存到本地..."
                else -> stringResource(id = R.string.syncing)
            }
            ...
            // 调试用：看看原始字符串到底是什么
//Text(text = "DEBUG: Raw Stage = ${state.currentStage}", style = MaterialTheme.typography.bodySmall)

            // 步进指示器：例如 (2/4)
            if (state.totalStages > 0) {
                Text(
                    text = "${state.stageIndex}/${state.totalStages}",
